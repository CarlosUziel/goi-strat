...
from data.utils import parallelize_map, run_cmd
...
def run_seacr(
    ...
    bed_path: Path,
    seacr_path: Path,
    control: Union[float, Path] = 0.01,
    norm_type: str = "norm",
    mode: str = "relaxed",
    pattern: str = "**/*.fragments.clean.ucsc.bedraph",
    slurm_kwargs: Optional[Dict[str, Any]] = None,
) -> None:
    """
    Run SEACR.

    *ref docs: https://github.com/FredHutch/SEACR/

    Args:
        seacr_src_file: Path to SEACR executable script.
        bed_path: Path to directory containing bedgraph files.
        seacr_path: Path to store peaks calls.
        control: Control (IgG) data bedgraph file to generate an empirical threshold for
            peak calling. Alternatively, a numeric threshold n between 0 and 1 returns
            the top n fraction of peaks based on total signal within peaks.
        norm_type: “norm” denotes normalization of control to target data, “non” skips
            this behavior. "norm" is recommended unless experimental and control data
            are already rigorously normalized to each other (e.g. via spike-in).
        mode: “relaxed” uses a total signal threshold between the knee and peak of the
            total signal curve, and corresponds to the “relaxed” mode described in the
            text, whereas “stringent” uses the peak of the curve, and corresponds to
            “stringent” mode.
        pattern: File name pattern the fastq files to be processed should follow.
        slurm_kwargs: A dictionary of SLURM cluster batch job options. If missing, each
            command will be run locally in sequential order.
    ...
    # 1. Get all fastq files per sample
    bed_files = dict()
    for bed_file in sorted(bed_path.glob(pattern)):
        sample_id = bed_file.stem.partition(".")[0]
        bed_files[sample_id] = bed_file

    # 2. Run bowtie2 for each sample
    logging.info("Submitting SLURM jobs:")
    for sample_id, bed_file in tqdm(bed_files.items()):
        # 2.1. Create intermediary paths
        sample_out_path = seacr_path.joinpath(sample_id)
        sample_out_path.mkdir(parents=True, exist_ok=True)
        ...
        cmd_args = map(
            ...
            [
                ...
                norm_type,
                mode,
                sample_out_path.joinpath(sample_id),
            ],
        )
        ...
        if slurm_kwargs is not None:
            slurm_kwargs["--error"] = str(
                sample_out_path.joinpath(f"{sample_id}.seacr.error.log")
            )
            slurm_kwargs["--output"] = str(
                sample_out_path.joinpath(f"{sample_id}.seacr.output.log")
            )
...
def run_nucl_panel_once(
    output_path: Path,
    sample_reads: Iterable[Path],
    nucl_panel: Dict[str, Any],
) -> None:
    """
    Count the appearences of nucleosome barcodes in paire-end reads.

    Args:
        output_path: Path to save count results.
        sample_reads: List of reads files for one sample.
        nucl_panel: Paired-end barcodes for each nuclesome (DNA barcode).
    """
    samples_nucl_reads = defaultdict(dict)

    # 1. Check DNA barcodes in first read
    with gzip.open(sample_reads[0], "rt") as fp:
        for line in fp:
            for nucl_id, nucl_barcodes in nucl_panel.items():
                samples_nucl_reads[nucl_id][("A", "R1")] = samples_nucl_reads[
                    nucl_id
                ].get(("A", "R1"), 0) + (nucl_barcodes["A"] in line)

                samples_nucl_reads[nucl_id][("B", "R1")] = samples_nucl_reads[
                    nucl_id
                ].get(("B", "R1"), 0) + (nucl_barcodes["B"] in line)

    # 2. Check DNA barcodes in second read
    with gzip.open(sample_reads[1], "rt") as fp:
        for line in fp:
            for nucl_id, nucl_barcodes in nucl_panel.items():
                samples_nucl_reads[nucl_id][("A", "R2")] = samples_nucl_reads[
                    nucl_id
                ].get(("A", "R2"), 0) + (nucl_barcodes["A"] in line)

                samples_nucl_reads[nucl_id][("B", "R2")] = samples_nucl_reads[
                    nucl_id
                ].get(("B", "R2"), 0) + (nucl_barcodes["B"] in line)

    # 3. Save results
    pd.DataFrame(samples_nucl_reads).transpose().stack(level=0).to_excel(
        output_path.joinpath("nucl_panel_counts.xlsx")
    )


def run_nucl_panel_counter(
    fastq_path: Path,
    panel_path: Path,
    nucl_barcodes_file_path: Path,
    ...
    Args:
        fastq_path: Path to directory containing fastq files.
        panel_path: Path to store matched counts.
        nucl_barcodes_file_path: File containing paired-end barcodes for each nuclesome.
        pattern: File name pattern the fastq files to be processed should follow.
    """
    # 0. Load panel of nucleosomes
    with nucl_barcodes_file_path.open("r") as fp:
        nucl_panel = json.load(fp)
    ...
    # 2. Count nucleosome reads per sample
    input_collection = []
    for sample_id, sample_reads in samples_reads.items():
        output_path = panel_path.joinpath(sample_id)
        ...
        input_collection.append(
            dict(
                ...
                nucl_panel=nucl_panel,
            )
        )

    if threads is not None and len(input_collection) > 1:
        parallelize_map(
            functools.partial(run_func_dict, func=run_nucl_panel_once),
            input_collection,
            threads=threads,
        )
    else:
        for ins in tqdm(input_collection):
            run_nucl_panel_once(**ins)


def run_bam_coverage_with_spike_in_norm(
    bam_path: Path,
    bam_coverage_path: Path,
    samples_norm_factors: Dict[str, float],
    bam_coverage_kwargs: Dict[str, Any],
    pattern: str = "**/*.bam",
    to_ucsc_chromosomes: bool = False,
    threads: int = 96,
    slurm_kwargs: Optional[Dict[str, Any]] = None,
) -> None:
    """
    Create bam coverage files (bigwig files) using spike-in normalization factors.

    Args:
        bam_path: Path to directory containing bam files.
        bam_coverage_path: Path to store normalized bigwig files.
        samples_norm_factors: Dictionary of sample_id and normalization factor value
            pair.
        bam_coverage_kwargs: A dictionary of bamCoverage options.
        pattern: File name pattern the .bam files to be processed should follow.
        to_ucsc_chromosomes: Whether to transform chromosome names to UCSC standard by
            adding a "chr" prefix.
        threads: Number of threads to use in samtools.
        slurm_kwargs: A dictionary of SLURM cluster batch job options. If missing, each
            command will be run locally in sequential order.
    """
    # 1. Get all bam files per sample
    ...
    # 2. Run bamCoverage for each sample
    logging.info("Submitting SLURM jobs:")
    for sample_id, bam_file in tqdm(bam_files.items()):
        if sample_id not in samples_norm_factors.keys():
            logging.warning(
                f"No normalization factor available for sample: {sample_id}"
            )
            continue

        # 2.1. Create intermediary paths
        sample_out_path = bam_coverage_path.joinpath(sample_id)
        sample_out_path.mkdir(parents=True, exist_ok=True)

        # 2.2. Build command
        samview = []
        input_bam_file = bam_file
        if to_ucsc_chromosomes:
            input_bam_file = bam_file.parent.joinpath(f"{bam_file.stem}.chr.bam")
            samview = [
                "samtools",
                "view",
                "--threads",
                ...
                threads,
                "-o",
                input_bam_file.with_suffix(".bai"),
                input_bam_file,
                "&&",
            ]
        ...
        cmd_args = map(
            ...
            [
                *samview,
                ...
                input_bam_file,
                "--scaleFactor",
                samples_norm_factors[sample_id],
                "-o",
                sample_out_path.joinpath(f"{sample_id}.bw"),
                *list(chain(*bam_coverage_kwargs.items())),
            ],
        )
        ...
        if slurm_kwargs is not None:
            slurm_kwargs["--error"] = str(
                sample_out_path.joinpath(f"{sample_id}.bamCoverage.error.log")
            )
            slurm_kwargs["--output"] = str(
                sample_out_path.joinpath(f"{sample_id}.bamCoverage.output.log")
            )
...
def run_compute_matrix_heatmap(
    bw_path: Path,
    heatmap_path: Path,
    gtf_file: Path,
    compute_matrix_kwargs: Dict[str, Any],
    pattern: str = "**/*.bw",
    slurm_kwargs: Optional[Dict[str, Any]] = None,
) -> None:
    """
    Compute scale-regions matrix and plot heatmap.

    Args:
        bw_path: Path to directory containing .bw files.
        heatmap_path: Path to store matrix and heatmap.
        gtf_file: Genome annotation file.
        pattern: File name pattern the .bam files to be processed should follow.
        slurm_kwargs: A dictionary of SLURM cluster batch job options. If missing, each
            command will be run locally in sequential order.
    """
    # 1. Setup
    bw_files = sorted(bw_path.glob(pattern))
    ...
    cmd_args = map(
        ...
        [
            ...
            "-S",
            *bw_files,
            *list(chain(*compute_matrix_kwargs.items())),
            "-o",
            heatmap_path.joinpath("matrix_gene.mat.gz"),
            "&&",
            "plotHeatmap",
            "-m",
            heatmap_path.joinpath("matrix_gene.mat.gz"),
            "-out",
            heatmap_path.joinpath("Histone_gene.png"),
            "--sortUsing",
            "sum",
        ],
    )
    ...
    if slurm_kwargs is not None:
        slurm_kwargs["--error"] = str(
            heatmap_path.joinpath("compute_matrix_heatmap.error.log")
        )
        slurm_kwargs["--output"] = str(
            heatmap_path.joinpath("compute_matrix_heatmap.output.log")
        )

        log_file = heatmap_path.joinpath("compute_matrix.sbatch.log")
        SlurmJobSubmitter(metadata=slurm_kwargs, tmp_dir=heatmap_path).submit(
            " ".join(cmd_args), "all_samples", log_file
        )
    else:
        log_file = heatmap_path.joinpath("compute_matrix.log")
        run_cmd(cmd=cmd_args, log_path=log_file)
