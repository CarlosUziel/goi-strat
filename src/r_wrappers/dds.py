...
def dml_fit_multi_factor(
    ...
    Args:
        bsseq_obj: An object of BSseq class for the BS-seq data.
...
def call_dml(dml_result: Any, **kwargs) -> Any:
    ...
    Args:
        dml_result: A data frame representing the results for DML detection.
            This should be the result returned from 'DMLtest' function.
...
def call_dmr(dml_result: Any, **kwargs) -> Any:
    """
    This function takes the results from DML testing procedure ('DMLtest' function)
    and calls DMRs. Regions will CpG sites being statistically significant are
    detected as DMRs. Nearby DMRs are merged into longer ones. Some restrictions
    including the minimum length, minimum number of CpG sites, etc. are applied.

    See: https://rdrr.io/github/haowulab/DSS/man/callDMR.html

    Args:
        dml_result: A data frame representing the results for DML detection.
            This should be the result returned from 'DMLtest' or
            'DMLtest.multiFactor' function.

    Returns:
        A data frame for DMRs. Each row is for a DMR. Rows are sorted by "areaStat",
        which is the sum of test statistics of all CpG sites in the region.
        The columns are:
